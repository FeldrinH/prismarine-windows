...
module.exports = (Item) => {
  return class Window extends EventEmitter {
    ...
    acceptCraftingClick (click) {
      if (click.mode !== 0) {
        // Hope the server handles shift-clicks and other complex click modes
        return
      }
      ...
      if (item && this.selectedItem &&
        item.type === this.selectedItem.type &&
        item.metadata === this.selectedItem.metadata) {
        // take crafting result and add to existing
        this.selectedItem.count += item.count
      } else {
        // create new item from crafting result
        this.selectedItem = new Item(item.type, itme.count, item.metadata, item.nbt)
      }
    }
